...
We next describe how the `nhanesA` package offers effective and reproducible solutions to computational
and analytical problems arising in epidemiological use of NHANES. 
...
[FIXME: Are there tests in place to identify whenever upstream changes make the translations
invalid?  If so they should be mentioned.]
...
[FIXME: does the package provide any guard rails to help avoid errors when users aren't cautious enough?]
...
Now, we perform the same analysis using the survey weights. First we get the adjusted overall mean for the population [FIXME: the table seems out of place.  The with/without survey weights comparisons can be aligned
in a single table perhaps]
