...
The publicly available continuous NHANES data consists of over 1000 different tables, or questionnaires. Each cycle surveys a distinct set of individuals using a cluster sample approach that is detailed here <https://wwwn.cdc.gov/nchs/nhanes/tutorials/SampleDesign.aspx>. Within each two year cycle surveyed individuals are given a unique identifier, `SEQN`, which can be used to link across questionnaires. Between cycles some questionnaires, such as Demography (DEMO), are repeated, and it is possible to join these based on the variables, but there can be changes to which variables are collected and in some instances the meaning of the variable may have changed.  

When working with NHANES data there are a small number of computational tasks that need to be addressed. FIXME:Laha - fill in your stuff here - maybe some details under each of the list items - or we could make each of these a sub heading and then have a description and some example code? That might be better.
...
We also offer functions to download NHANES data using R and load it into dataframes for subsequent analysis.

- align tables within a cycle or across cycles
After downloading the data, we synchronize tables from different surveys (e.g., demography and dietary) within a cycle. Additionally, we can align tables across years, such as concatenating the demography survey data from 2009-2010, 2011-2012, and 2013-2014.

- translate tables - to avoid integer codes

Categorical variables, both ordered and unordered, are encoded as integers. In many scenarios, using integers is not intuitive. Thus, we provide functions to convert these integers to their actual values, enabling researchers to work with more meaningful data

- use the survey weights to obtain valid estimates

When using NHANES data, samples are weighted to ensure they represent the U.S. civilian noninstitutionalized resident population. Each sample person is assigned a sample weight, reflecting the number of people in the broader population that the individual represents. To obtain valid estimates from the data, it's essential to apply these survey weights during analysis. By doing so, researchers account for the complex survey design and potential biases, ensuring the results are reflective of the entire population and not just the sampled individuals. Utilizing software or statistical packages (eg survey) that support survey-weighted analysis, one can produce unbiased national estimates and accurate standard errors.
...
## Example
Here we will demonstrate the use of the `nhanesA` package, together with the `survey` package to look at average blood pressure for individuals over 40 years of age by reported ethnicity for the 2017-2018 cycle.  To do that we need to obtain the demographic data (DEMO_J) and the blood pressure data (BXP_J) and merge them.
...
We will examine the diasotolic blood pressure and for ease of presentation we only use the first measurement, variable BPXDI1 in tabel BPX_J.

```{r ethtables}
table(datasub$RIDRETH1)
##divide the data by
...
Now, we perform the same analysis using the survey weigths.
...
cbind(mns, adjmnsbyEth$BPXDI1)
# By Gender 
svyby(~BPXDI1, ~RIAGENDR, dfsub, svymean, na.rm=TRUE)
