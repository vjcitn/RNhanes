...
Since 1999, the NHANES survey has been conducted continuously, and the surveys during that period are referred to as "continuous NHANES" to distinguish from several prior surveys. Continuous NHANES surveys are grouped in two-year *cycles*, with the first cycle carried out in 1999-2000.  For the most part the data are public and are very widely used for a variety of different analysis.  In fact, searching PubMed for the published papers that cite NHANES shows that currently more than 5,000 papers per year make some use of this resource. The bulk of the NHANES data are available for download from the CDC website. Detailed instructions on using the data, how to download it and analyze it are given.

Other benefits: 
 - different ethnicities
 - ages ranging from babies to the elderly
 - oversampling some of the rarer populations
 - why is NHANES influential - trends in obesity, social determinants of health research
 - another challenge is appropriate use of survey weights in analysis
 - it provides a resource for identifying changes in trends over time - so if we look at obesity, can we show any slow down or remediation in a more recent version - that is simplified if we have easy access to any previous results
...
The publicly available NHANES data consists of about 1000 different tables, or questionnaires. Within each two year cycle the questionnaires refer to the same set of individuals. Each cycle surveys a distinct set of individuals using a cluster sample approach that is detailed here <https://wwwn.cdc.gov/nchs/nhanes/tutorials/SampleDesign.aspx>. Within each cycle individuals are given a unique identifier, `SEQN`, which can be used to join questionnaires. Between cycles some questionnaires, such as Demography (DEMO), are repeated, but there can be changes to which variables are collected. There

Challenges:
1) align tables within a cycle
2) align tables across cycles
3) translate tables - to avoid integer codes
4) survey weights - need to use them to obtain valid estimates

The `nhanesA` package provides a set of functions in the R programming language that can be used to search for variables and questionnaires, download data to the users computer, and analyze the data.
...
## Confict of interest.
RG consults broadly in the pharmaceutical and Biotech industries.
