...
The publicly available continuous NHANES data consists of over 1,000 distinct tables. These tables encompass a variety of data, including results from detailed questionnaires. Each cycle surveys a unique group of individuals using a cluster sample methodology that is detailed here [NHANES Sample Design Tutorial](https://wwwn.cdc.gov/nchs/nhanes/tutorials/SampleDesign.aspx). The data from each NHANES cycle is categorized into five publicly accessible sections: Demographics, Dietary, Examination, Laboratory, and Questionnaire. Some data sets remain restricted, accessible only through a formal request. 


Data from the CDC can be accessed through HTTPS requests on their website. Each dataset comprises two primary components:

1. The raw data, presented in the SAS (Statistical Analysis Software) 'XPT' (Transport File Format).
2. A complementary documentation file in HTML format detailing the data variables and their structure.
...
We next describe a number of the computational and analytic challenges inherent to this process, highlighting how the `nhanesA` package offers effective and reproducible solutions. For a full list and description of `nhanesA` functions refer to <INCLUDE LINK>.

**FIXME: Chris: Discussion of pre-pandemic seems out of place here. I suggest moving this to the discussion**


### Search relevant variables and data files for analysis

While the CDC website provides search capabilities, using dedicated R-based tools offers more direct manipulation of research results. Within the `nhanesA` package, we have incorporated advanced search utilities. Functions such as `nhanesSearch`, `nhanesSearchTableNames`, and `nhanesSearchVarName` have been crafted streamline and optimize these search processes.

### Downloading data to your local machine


The data is downloaded directly into data frames, readying it for subsequent analysis. The function `nhanes` takes the name of the table that is wanted and downloads it. Users can specify whether the data should be translated at this point, by setting the argument `translated` to `TRUE`.  However, we advise translating the data by default, so the translated argument is set to TRUE as standard.

As an illustration, raw gender codes like 1 and 2 will be translated into 'Male' and 'Female' respectively. Categorical variables, both ordinal and nominal, are represented in the raw data as integers. In many scenarios, using integers is not intuitive, potentially leading to rather uninterpretable models. In Figure \ref{fig:DEMO_J} displays the difference between the translated and untranslated variables RIAGENDR and RIDRETH1 from DEMO_J. If one were to use the untranslated variable in a regression analysis, for example, it would be treated as an integer variable. However, it represents a categorical variable, and needs to be encoded accordingly.


### Align tables within a cycle or across cycles

After downloading the data, tables from different surveys (e.g., demographic and dietary) can be synchronized within a cycle using the `merge` function from R with `SEQN` as the join key. Additionally, one can align tables across years, such as concatenating the demographic survey data from 2009-2010, 2011-2012, and 2013-2014. When aligning across cycles, users should be careful to check that selected variables are present in all cycles and have common definitions. For example, the `RIDAGEYR` variable is present in the cycles mentioned and thus could be included when joining across cycles. 
...
Within NHANES there is a substantial amount of missing data. In part this arises from non-response but it can also arise due to the fact that not all respondents participate in all of the assays, exams or questionnaires. 
Some questionnaires are reused across cycles. For example the main demographic questionnaire is present in all cycles. The CDC uses some naming conventions, but these are not always applied consistently and it is important to be cautious when merging or combining data across questionnaires or cycles. The names of the variables are not guaranteed to be constant, and the actual questions may change over time. 

The survey questions are delivered sequentially and there is some amount of structure that will need to be dealt with. In the Blood Pressure and Cholesterol questionnaire for 2005-2006 we show an excerpt of the Data Documentation file in Figure. 2. You can see that in question BPQ_020 that anyone who answered either ‘No’ or ‘Don’t know’ to the question skipped over question BPQ_030, as it makes little sense for them. Importantly the value stored in the database for those people for that question was a missing value. Now, in some circumstances an analyst might prefer to assume that if the respondent had not been told that they had high blood pressure once, they also had not been told they had high blood pressure two or more times. There are many instances in the NHANES data where questions are skipped as part of the survey delivery and it is important that the analyst try to detect those and make reasonable assumptions for the analysis. 
...
**FIXME: Chris: I don't see the point in mentioning these packages so prominently. They are not viable alternatives to nhanesA. **
...
**FIXME: Chris, Should this be bullet points? It's not meant to be an ordered list.**
...
Here we sketch out an outline for writing a paper using the tools we mention in order to create a reproducible paper. By reproducible we really mean that once we have agreed on the data to use, that all of the tables, graphs and other data analyses can be reproduced exactly. Now this is not the concept of scientific reproducibility where one expects to find a similar result when the basic experiment is repeated on similar but not identical populations, but it is an important goal in and of itself. **FIXME: Chris, I find this confusing to present here. We can discuss**
